...
class Queue {
   ...
   add(job) {
      debugLog(debugEnabled, 'klyft', `preparing ${job.data.id} on worker ${this.pid}`)
   ...
   start(onThread) {
      ...
      if(jobsAvailable) {
         ...
         debugLog(debugEnabled, 'klyft', `starting ${job.data.id} on thread ${this.threads[onThread].fork.pid}`)
